...
my $SmallL = $ARGV[0];                                                            # reads in command line arguments
my $SmallM = $ARGV[1];
my $LargeL = $ARGV[2];
my $LargeM = $ARGV[4];
my %Mass = (
...
foreach my $vol ($SmallL, $LargeL) {
  ...
  foreach my $f (@files) {                                                        # loops through matching files
    ...
    foreach my $l (@lines) {                                                      # loops through matched lines
      ...
      if (!grep{$_ eq $split[1]} @t) {                                            # fills the smearing time array
        push(@t, $split[1]);
      }
  ...
  foreach my $beta (@{$Beta{$vol}}) {
    foreach my $loop (0,1,2,3,4) {                                                # loops over observables
      foreach my $b (@{$block{$vol}}) {                                           # beta values
        foreach my $t (@t) {                                                      # and smearing times
...
foreach my $block_level (@{$block{$LargeL}}) {                                    
  if ($block_level == 0) {next;}                                                  # skips no matching on the large volume
  foreach my $t (@t) {
    foreach my $loop (0,1,2,3,4) {
      foreach my $largeb (@{$Beta{$LargeL}}) {                                    # loops over large volume beta
        my $lv_value = $avg{$vol}{$largeb}{$t}{$loop}{$b}                                  # large volume value at large volume beta
        my @x1 = @{$Beta{$SmallL}};                                               # arrays for plots
        my @y1 = ();
        my @e1 = ();
        my @x2 = @{$Beta{$LargeL}};
        my @y2 = ();
        ...
        my $n_diff;
        my $o_diff=100;
        foreach my $smallb (@{$Beta{$SmallL}}) {                                  # finds area near intersection
          $n_diff = $avg{$SmallL}{$smallb}{$t}{$loop}{$b} - $lv_value;
          if (($n_diff <= $o_diff) && ($n_diff >= 0)) {
            $o_diff = $n_diff;
            $index  = $count;
          }
          $count++;
          push(@y1, $avg{$SmallL}{$smallb}{$t}{$loop}{$b});
          push(@e1, $err{$SmallL}{$smallb}{$t}{$loop}{$b});
        }
        foreach my $lv_beta (@{$Beta{$LargeL}}) {
          push(@y2, $avg{$LargeL}{$largeb}{$t}{$loop}{$b});
          push(@e2, $err{$LargeL}{$largeb}{$t}{$loop}{$b});
        }
        ...
        elsif ($index > (@{$Beta{$SmallL}}-3)) {
          @x = @{$Beta{$SmallL}}[(@{$Beta{$SmallL}}-4)..(@{$Beta{$SmallL}}-1)];
          @y = @y1[(@y1-4)..(@y1-1)];
          @e = @e1[(@e1-4)..(@e1-1)];
        }
        else{
          @x = @{$Beta{$SmallL}}[($index-1)..($index+2)];
        ...
        my $chart = Chart::Gnuplot->new(                                          # Create chart object 
          ...
          title  => "Deltabeta for beta ${largeb}, matching with ${LargeL} blocked ${block_level} after ${t} smearing",
          xlabel => "Beta",
        ...
        $chart->command("set arrow from $largeb,$lv_value to $beta_diff,$large");
...
foreach my $largeb (@{$Beta{$LargeL}}) {
  ...
  foreach my $loop (0,1,2,3,4) {
    ...
    foreach my $t (@t) {
      ...
      $count ++;
      push(@y1,$Full_delta_beta{1}{$largeb}{$label}{$t});
      push(@y2,$Full_delta_beta{2}{$largeb}{$label}{$t});
      push(@y3,$Full_delta_beta{3}{$largeb}{$label}{$t});
    }

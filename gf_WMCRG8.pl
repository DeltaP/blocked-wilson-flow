...
foreach my $vol ($SmallV, $LargeV) {
  my $base_name = "${NF}flav_${vol}/WMCRG8_";
  my @files = grep { /WMCRG8_(high|low)_${vol}_[0-9].[0-9]_-0.25_$Mass{$vol}/ } glob( "$base_name*" );           # globs for file names
    ...
    foreach my $loop (0,1,2,3,4) {
      foreach my $largeb (@{$Beta{$LargeV}}) {                                    # loops over large volume beta
        ...
        my $chart = Chart::Gnuplot->new(                                          # Create chart object 
          output => "Plots_${NF}_WMCRG8/deltabeta/${largeb}_${block}_${t}_${loop}_full.png",
...
foreach my $largeb (@{$Beta{$LargeV}}) {
  ...
  my $chart = Chart::Gnuplot->new(                                              # Create chart object 
    output => "Plots_${NF}_WMCRG8/avg_smearing_time/avg_${largeb}.png",
    ...
    open FILE, ">", "Plots_${NF}_WMCRG8/out/avg_${largeb}" or die $!;

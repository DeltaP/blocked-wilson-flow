...
foreach my $vol ($SmallV, $LargeV) {
  my $base_name = "${NF}flav_${vol}/BlockedWflow7_";
  my @files = grep { /BlockedWflow7_(high|low)_${vol}_[0-9].[0-9]_-0.25_$Mass{$vol}/ } glob( "$base_name*" );           # globs for file names
    ...
    foreach my $loop (0,1,2,3,4) {
      foreach my $largeb (@{$Beta{$LargeV}}) {                                    # loops over large volume beta
        ...
        my $chart = Chart::Gnuplot->new(                                          # Create chart object 
          output => "Plots_${NF}flav7/deltabeta/${largeb}_${block}_${t}_${loop}_full.png",
...
foreach my $largeb (@{$Beta{$LargeV}}) {
  ...
  foreach my $loop (0,1,2,3,4) {
    ...
    my $chart = Chart::Gnuplot->new(                                              # Create chart object 
      output => "Plots_${NF}flav7/smearing_time/${largeb}_${loop}.png",
    ...
    open FILE, ">", "Plots_${NF}flav7/out/${largeb}_${loop}" or die $!;
...
foreach my $largeb (@{$Beta{$LargeV}}) {
  ...
  foreach my $loop (0,1,2,3,4) {
    ...
    my $chart = Chart::Gnuplot->new(                                              # Create chart object 
      output => "Plots_${NF}flav7/same_smearing_time/${largeb}_${loop}.png",
      title  => "Delta Beta as a function of smearing time for Beta: ${largeb}",
      xlabel => "Smearing Time",
      ylabel => "Delta Beta",
      #yrange => [0,1]
...
foreach my $largeb (@{$Beta{$LargeV}}) {
  ...
  my $chart = Chart::Gnuplot->new(                                              # Create chart object 
    output => "Plots_${NF}flav7/avg_smearing_time/avg_${largeb}.png",
    title  => "Delta Beta as a function of smearing time for Beta: ${largeb}",
    xlabel => "Smearing",
    ylabel => "Delta Beta",
    #yrange => [0,1]
    ...
    open FILE, ">", "Plots_${NF}flav7/out/avg_${largeb}" or die $!;

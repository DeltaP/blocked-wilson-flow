...
foreach my $vol ($SmallV, $LargeV) {
  my $base_name = "${NF}flav_${vol}/BlockedWflow_${vol}_*_-0.25_$Mass{$vol}.";
    ...
    foreach my $loop (0,1,2,3,4) {
      foreach my $largeb (@{$Beta{$LargeV}}) {                                    # loops over large volume beta
        ...
        my $chart = Chart::Gnuplot->new(                                          # Create chart object 
          output => "Plots_oldaction${NF}flav/deltabeta/${largeb}_${block}_${t}_${loop}_full.png",
...
foreach my $largeb (@{$Beta{$LargeV}}) {
  ...
  foreach my $loop (0,1,2,3,4) {
    ...
    my $chart = Chart::Gnuplot->new(                                              # Create chart object 
      output => "Plots_oldaction${NF}flav/smearing_time/${largeb}_${loop}.png",
